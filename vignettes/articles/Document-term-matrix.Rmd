...
```{r}
taantuvat <- sf::st_drop_geometry(taantuvat)
y <- taantuvat[c("kunta_nimi", "suht_muutos_2010_2022")] |>
  inner_join(lemmas[c("kunta")], by = join_by("kunta_nimi" == "kunta")) |>
  group_by(kunta_nimi) |>
  slice(1)
```
...
```{r}
data <- left_join(lemmas, y, by = join_by("kunta" == "kunta_nimi")) |>
