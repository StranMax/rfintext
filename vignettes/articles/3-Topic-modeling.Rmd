...
environment with 40 core machine with 177 Gb of RAM this took only ~10min. 
...
Visually selecting potentially best k values. With values 5 and 15 different iterations 
...
```{r}
ptm <- proc.time()
selected_models <- lda_models |> filter(K %in% optimal_k) |>
  mutate(
...
final_models |>
  ...
  ggplot(aes(x = beta, y = term, fill = K, colour = is_unique)) +
