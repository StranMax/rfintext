...
Visually selecting potentially best k value/values. Coherence decline seems to 
stop and even slightly increase when k is over 10. Let's pick value 13 for k
since there is also less variation.
...
different models. Similarity of certain tpoics between different models could 
...
topic_pairs |> select(K1, K2, r) |>
  ...
  mutate(topic1 = as.factor(as.integer(topic1)),
         mean_similarity_by_ntopics = mean_similarity*K1) |>  # TODO: similarity depends somehow on number of topics
  ggplot() +
...
Figure would suggest that topics 4, 5, 10 and 12 could be particularly solid 
choices. 
...
Here we choose the final model(s)
...
Topics 1, 2, 3, 5 and 12 are most common. Rarest is topic 10.
...
What is the share of the largest topic for document?
...
It seems that especially topics 4, 5, 9, 10 and 13 are sort of main topics which 
define whole document where as for example topics 3, 8 and 13 seem to cover only 
half of the document. There are a few exceptions, For example Kuusamo document 
consists of almost exclusively topic 3. 
...
Here we take a look on topics and what they are.
